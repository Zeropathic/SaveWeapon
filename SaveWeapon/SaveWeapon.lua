...
mod.delete_item = function(backend_id)
	...
	--mod.more_items_library:remove_mod_items_from_local_backend({backend_id}, "SaveWeapon")
...
mod.load_items = function()
	...
	for save_id, savestring in pairs(mod.saved_items) do
		...
		then
			...
			table.insert(mod.created_items, created_entry) -- Add backend ID to the list of items created this session
...
mod:hook_safe(mod.more_items_library, "add_mod_items_to_local_backend", function(self, items, mod_name)
	...
	then
		for item_num = 1, #items do
			...
			local entry = {
				backend_id = item.mod_data.backend_id,
				savestring = savestring,
			}
			table.insert(mod.created_items, 1, entry) -- Add item backend to the list of items created this session
		end
	end
end)
